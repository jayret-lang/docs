...
@examples{@"@"Check void test() {
    ...
    assertEquals(not(2 < 1), true);
}}
  @section{Boolean Operators}

@form["and" "left and right"]{
  @margin-note{
  The @pyret{&&} operator “short-circuits”: if @pyret{left}
  ...
  @examples[#:show-try-it #t]{@"@"Check void test() {
    ...
    assertEquals((4 < 5) && ("a" == "b"), false);
    assertEquals((4 < 5) && ((4 + 2) == 6), true);
    assertEquals((4 > 5) && ((4 + 2) == 6), false);
}}
}
