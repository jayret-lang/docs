...
of them, and these use cases (and others) motivate Jayret's.

This section describes the components of Jayret's module system and how to use
it.
...
@item{How to tell Jayret to @emph{locate} one module from a different module}
@item{How to @emph{include} names from a located module}
]

Here's a simple example that demonstrates all three.

In a file called @tt{list-helpers.jrt}:

@pyret-block{// [Jayret] explicit `provide`: 
/* contract: concat :: Object */;
Object concat(l1, l2) {
    return l.append(l1, l2);
}}

In a file in the same directory called @tt{list-user.jrt}:

@pyret-block{import file("list-helpers.jrt")
@"@"Check void test() {
    assertEquals(concat([1, 2], [3, 4]), [1, 2, 3, 4]);
}}

The @pyret{// [Jayret] explicit `provide`:} declaration tells Jayret to make @emph{all} the names of
functions and values defined in the @tt{list-helpers.jrt} module available to
any module that imports it. The @pyret{import file("list-helper.jrt")}
declaration tells Jayret to find the module at the path given in quotes, and
then make all the names it provides available in the top-level of the module
the @pyret{include} appears in.

In general, @pyret{include} and @pyret{// [Jayret] explicit `provide`:} are handy ways to provide a
collection of definitions to another context across modules.

Note that @pyret{// [Jayret] explicit `provide`:} only provides @emph{values}.  If you want to
provide @emph{type definitions}, then you may use a related declaration:

In a file called @tt{mypos-provider.jrt}:

@pyret-block{// [Jayret] explicit `provide`: 
// This line provides `pos2d`, `pos3d`, `is-pos2d` and `is-pos3d`
// This line provides the type named `MyPos`
data MyPos {
    Pos2d(x, y);
    Pos3d(x, y, z);
}
...
In a file in the same directory called @tt{mypos-user.jrt}:

@pyret-block{import file("mypos-provider.jrt")
switch (favorite-spot) {
    case Pos2d(x, y): yield "Two dimensions";
    case Pos3d(x, y, z): yield "Three dimensions";
}}

The @; TODO(pyret2jayret): parse failed (no shifts)
@pyret{type *} declaration is needed for the @pyret{cases}
...
@pyret{include} statement tells Jayret how to find the module.
...
work in @url{jayret-lang.github.io/code}, where it is assumed the user is authenticated to
Google Drive.

The meaning of the currently supported forms are:

@form["<id-import>" "<id-of-builtin>"]{
@pyret-block[#:style "good-ex"]{import string-dict
}
...
@form["file" "file(<path>)"]{
@pyret-block[#:style "good-ex"]{import file("path/to/a/file.jrt")
}
...
@pyret-block[#:style "good-ex"]{import my-gdrive("stored-in-gdrive.jrt")
}
Looks for a Jayret file with the given filename in the user's
@tt{jayret-lang.github.io/code/} directory in the root of Google Drive.
}

@form["shared-gdrive" "shared-gdrive(<name>, <id>)"]{
@pyret-block[#:style "good-ex"]{import shared-gdrive("public-in-gdrive.jrt", , "ABCDEF12345")
}
Looks for a Jayret file with the given id in Google Drive. The file must have
...
using the “Publish” menu from @tt{jayret-lang.github.io/code}
}

@form["url" "url(<url>)"]{
@pyret-block[#:style "good-ex"]{import url("https://raw.githubusercontent.com/brownplt/pyret-lang/refs/heads/horizon/tests/io-tests/tests/library-code.jrt") as L
}
Look for a Jayret file at the given URL. For loading files from URLs in the
browser (e.g. on jayret-lang.github.io/code), the server must support
@hyperlink["https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/CORS#simple_requests"]{simple
CORS}. From the command-line, any publicly available URL that serves Jayret code
in a text/plain response is an appropriate target.

Browsers, jayret-lang.github.io/code, and command-line Jayret @emph{may} cache the result of
...
the Visual Studio Code Extension, this option can be set from the Jayret settings
menu.
...
To this end, Jayret supports several forms for controlling names of various
kinds:
...
In @secref["s:modules:quick-start"] we showed @pyret{// [Jayret] explicit `provide`:} and
...
Object concat(list-of-lists) {
    return [for fold(acc : empty, l : list-of-lists) { yield acc.append(l); }];
}
Object is-odd-length(l) {
    return ...;
}}
...
@pyret-block{import file("list-helpers.jrt") as LH1
import file("list-helpers2.jrt") as LH2
@"@"Check void test() {
    assertEquals(LH1.concat([1, 2], [3, 4]), [1, 2, 3, 4]);
    assertEquals(LH2.concat([[1, 2], [3]]), [1, 2, 3]);
}}
...
For situations where these issues become too onerous, Jayret provides more ways
to control names.
...
provide fewer names than @pyret{// [Jayret] explicit `provide`:}, a module can use one or more
...
@pyret-block{// This module provides the same exports as the one above
// [Jayret] explicit `provide`: 
Object concat-helper(element, lst) {
    return ...;
}
Object concat(list1, list2) {
    return ...;
}
Object is-odd-length(l) {
    return ...;
...
@pyret-block{// This module provides two names: is-odd-length and append
// [Jayret] explicit `provide`: 
Object concat-helper(element, lst) {
    return ...;
}
...
@pyret-block{import string-dict as SD
// [Jayret] explicit `provide`: 
// [Jayret] explicit `provide`: 
Object my-string-dict-helper() {
    return ...;
}}
...
@pyret-block{import tables as T
import chart as C
import image as I
// [Jayret] explicit `provide`: 
// [Jayret] explicit `provide`: 
...
@pyret-block{// A module that includes this one will see the name NumPair as a type
// [Jayret] explicit `provide`: 
type NumPair = {Number ;Number }}

Just as with @prod-link['PyretModules]{provide-value-spec} above, a
@prod-link['PyretModules]{provide-type-spec} can use @; TODO(pyret2jayret): parse failed (no shifts)
@pyret{type *} as shorthand to
...
@pyret-block{// A module that includes this one will see the name SD as a module id,
...
As with values and types, a module may use @; TODO(pyret2jayret): parse failed (no shifts)
@pyret{data *} as a shorthand to
...
name for the type alias (e.g. @; TODO(pyret2jayret): parse failed (no shifts)
@pyret{type D1 as D2}) if you want to refer to
...
@pyret-block{// This program puts just two names from the builtin string-dict module into
// scope.
import string-dict as SD
import fromSD:mutable-string-dict,make-mutable-string-dictend
}

@pyret-block{// This program imports and renames two values from the string-dict module
import string-dict as SD
import fromSD:mutable-string-dict as dict,make-mutable-string-dict as make-dictend
}

@pyret-block{// This program includes all the value names from the string-dict module
import string-dict as SD
import fromSD:*end
}
...
import string-dict as SD
import fromL:mapend
import fromSD:mutable-string-dict as mapend
}
...
@pyret-block[#:style "bad-ex"]{import file("mypos-provider.jrt") as P
import fromP:*,type *end
switch (pos2d(3, 4)) {
    case Pos2d(x, y): yield ...;
    case Pos3d(x, y, z): yield ...;
}}

However, the following example works just fine:

@pyret-block[#:style "good-ex"]{import file("mypos-provider.jrt") as P
import fromP:*,data *end
switch (pos2d(3, 4)) {
    case Pos2d(x, y): yield ...;
    case Pos3d(x, y, z): yield ...;
}}
...
Jayret used to have, and supports for backwards compatibility, a few other
...
@tabular[#:sep (hspace 4)
 (list
  ...
  (list @pyret-block{// [Jayret] explicit `provide`: 
}
        @pyret-block{// [Jayret] explicit `provide`: 
})
  (list @pyret-block{// [Jayret] explicit `provide`: 
  ...
  (list @; TODO(pyret2jayret): parse failed (no shifts)
@pyret-block{include <some-module>}
        @; TODO(pyret2jayret): parse failed (no shifts)
@pyret-block{
  ...
  (list @; TODO(pyret2jayret): parse failed (no shifts)
@pyret-block{import name1, ... from <some-module>}
        @; TODO(pyret2jayret): parse failed (no shifts)
@pyret-block{
        import <some-module> as <some-name>
        include from <some-name>:
