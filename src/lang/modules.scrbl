...
work in @url{code.jayret.org}, where it is assumed the user is authenticated to
Google Drive.
...
@tt{code.jayret.org/} directory in the root of Google Drive.
}
...
using the “Publish” menu from @tt{code.jayret.org}
}
...
browser (e.g. on code.jayret.org), the server must support
...
Browsers, code.jayret.org, and command-line Jayret @emph{may} cache the result of
